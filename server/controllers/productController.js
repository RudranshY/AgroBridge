...
// Add Product (defensive)
const addProduct = async (req, res) => {
  try {
    // seller id from auth middleware
    req.body.sellerId = req.sellerId;

    // file from multer: uploadedImage.buffer exists
    const uploadedImage = req.file;
    console.log("uploadedImage:", uploadedImage);

    // If missing image -> bad request
    if (!uploadedImage) {
      return res.status(400).json({ message: "No image uploaded" });
    }

    // If client posted FormData, location may be a JSON string -> try parse
    if (req.body.location && typeof req.body.location === "string") {
      try {
        req.body.location = JSON.parse(req.body.location);
      } catch (e) {
        console.warn("Could not parse req.body.location JSON:", e);
      }
    }

    // If client sent lat/lng fields separately, populate location
    if (
      (!req.body.location || !Array.isArray(req.body.location.coordinates)) &&
      (req.body.longitude || req.body.lat || req.body.lng)
    ) {
      const lng = parseFloat(req.body.longitude ?? req.body.lng ?? 0);
      const lat = parseFloat(req.body.latitude ?? req.body.lat ?? 0);
      if (!Number.isNaN(lng) && !Number.isNaN(lat)) {
        req.body.location = { type: "Point", coordinates: [lng, lat] };
      }
    }

    // Brand fallback: from body -> cookie -> seller (if you store) -> default
    if (!req.body.brand) {
      req.body.brand = req.cookies?.brandName || req.body.sellerName || "Local Seller";
    }

    // Cloudinary upload: if fails return early (important)
    try {
      const cloudRes = await uploadImageToCloudinary(uploadedImage.buffer);
      if (!cloudRes || !cloudRes.secure_url) {
        console.error("Cloudinary unexpected response:", cloudRes);
        return res.status(500).send({ message: "Image upload failed (Cloudinary)." });
      }
      req.body.image = cloudRes.secure_url;
    } catch (err) {
      console.error("Cloudinary upload error:", err);
      return res.status(500).send({
        message: "There was a problem communicating with Cloudinary during the image upload.",
      });
    }

    // Validate minimal required fields before saving
    if (!req.body.brand) {
    ...
    if (!req.body.location || !Array.isArray(req.body.location.coordinates) || req.body.location.coordinates.length !== 2) {
      return res.status(400).send({ message: "location.coordinates [lng,lat] is required" });
    }
